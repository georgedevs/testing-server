...
exports.initSessionCleanup = void 0;
...
const initSessionCleanup = () => {
    ...
    node_cron_1.default.schedule('*/5 * * * *', async () => {
        try {
            ...
            const expiredGraceSessions = await bookingModel_1.Meeting.find({
                graceActive: true,
                graceEndTime: { $lt: now },
                status: 'confirmed'
            });
            for (const meeting of expiredGraceSessions) {
                ...
                if (completionStatus !== 'incomplete') {
                    ...
                    if (client) {
                        ...
                        const existingSession = client.sessionHistory.find(session => session.counselorId.toString() === meeting.counselorId?.toString() &&
            ...
            const overdueSessions = await bookingModel_1.Meeting.find({
                status: 'confirmed',
            ...
            for (const meeting of overdueSessions) {
                ...
                try {
                    ...
                    if (typeof meeting.meetingDate === 'string') {
                        const dateString = meeting.meetingDate;
                    ...
                    // Handle Date object type
                        ...
                        const dateObj = meeting.meetingDate;
...
exports.initSessionCleanup = initSessionCleanup;
